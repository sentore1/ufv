...
export default function ContactPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data } = await supabase.from('content_sections').select('*').eq('section_key', 'contact_info').single();
  ...
  const info = contactInfo?.content?.[locale]?.split('|') || ['', '', '', ''];
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-20">
        ...
        <div className="grid md:grid-cols-2 gap-16">
          ...
          {/* Contact Info */}
          <div>
            <h2 className="text-4xl font-bold mb-6 text-gray-900">{t('getInTouch')}</h2>
            <p className="text-xl text-gray-600 mb-12">{t('getInTouchDesc')}</p>
            ...
            <div className="space-y-8">
              <div className="flex items-start gap-6">
                ...
                <div>
                  <h3 className="font-bold text-xl mb-2 text-gray-900">{t('officeLocation')}</h3>
                  <p className="text-gray-600 text-lg">{info[0]}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-6">
                ...
                <div>
                  <h3 className="font-bold text-xl mb-2 text-gray-900">{t('email')}</h3>
                  <p className="text-gray-600 text-lg">{info[1]}<br/>{info[2]}</p>
                </div>
              </div>
            ...
            <div className="mt-12">
              <h3 className="font-bold text-xl mb-4 text-gray-900">{t('followUs')}</h3>
          ...
          <div className="bg-gray-50 p-12 rounded-3xl shadow-xl">
            <h3 className="text-3xl font-bold mb-8 text-gray-900">{t('sendMessage')}</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-gray-700 mb-3 font-semibold text-lg">{t('name')}</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="block text-gray-700 mb-3 font-semibold text-lg">{t('email')}</label>
                <input 
                  type="email" 
                  required
                  className="w-full px-6 py-4 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-green-600 transition-colors"
              ...
              <div>
                <label className="block text-gray-700 mb-3 font-semibold text-lg">{t('subject')}</label>
                <input 
                  type="text" 
                  required
                  className="w-full px-6 py-4 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-green-600 transition-colors"
              ...
              <div>
                <label className="block text-gray-700 mb-3 font-semibold text-lg">{t('message')}</label>
                <textarea 
                  rows={6} 
                  required
