    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-full whitespace-nowrap ${selectedCategory === cat ? "bg-orange-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>
              {cat}
            </button>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogs.map((blog) => (
            <div key={blog.id} className="bg-white rounded-lg shadow hover:shadow-lg transition overflow-hidden">
              ...
              <div className="p-6">
                {blog.category && <span className="text-xs bg-orange-100 text-orange-800 px-2 py-1 rounded">{blog.category}</span>}
                ...
                <Link href={`/blog/${blog.slug || blog.id}`} className="text-orange-600 hover:text-orange-700 font-semibold">Read More →</Link>
              </div>
            </div>
          ))}
        </div>
